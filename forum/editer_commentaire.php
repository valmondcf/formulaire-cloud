...
$req = $pdo->prepare("SELECT * FROM topic_commentaire WHERE id = ? ");
...
if(!empty($_POST)){
    ...
    if(isset($_POST['modification'])){
        ...
        $valid = false;
        $err_commentaire = "Le commentaire ne peut pas être vide";
    
        }elseif(mb_strlen($commentaire) < 4){
            $valid = false;
        ...
        if($valid){
            ...
            $req = $pdo->prepare("UPDATE topic_commentaire SET contenu = ?, date_modification = ? WHERE id = ?");
